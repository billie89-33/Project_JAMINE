...
**วิธีแก้:** Frontend ต้องไม่ยอมให้ส่ง Request และ Backend ต้องครอบ `if` เพื่อป้องกันข้อมูลหลุด
```javascript
// ✅ Backend Validation
if (status === 'Shipped') {
    if (!trackingNumber || trackingNumber.trim() === '') {
        throw new Error('กรุณาระบุเลขพัสดุจากบริษัทขนส่งก่อนยืนยันการจัดส่ง'); // บังคับข้อมูลสำคัญ
    }
...
**วิธีแก้:** แทนที่จะเปลี่ยนหน้า (`navigate('/order/:id')`) ซึ่งทำให้ผู้ใช้เสียตำแหน่งเดิม ให้ใช้ **Modal (Popup)** แทน สิ่งนี้ช่วยให้ UX ลื่นไหล และผู้ใช้สามารถกดดูออเดอร์หลายๆ อันสลับอันไปมาได้ง่ายขึ้น

### 4. UI Resilience for External Data (ความอึดถนึกของ UI ต่อการเปลี่ยนแปลง)
...
**วิธีแก้:** สร้าง Object/Array ในส่วน `shared/constants/` ของ Frontend และส่วน Backend ให้ตรงกันเป๊ะ 100% ห้ามพิมพ์ String สดๆ (Hardcode) เด็ดขาด เพื่อป้องกัน `Mongoose ValidationError` เมื่อ Backend เพิ่มสถานะใหม่
...
**บทเรียน:**
เมื่อมีการเพิ่ม API endpoint ใหม่ในไฟล์ Service **ต้องตรวจสอบการ `export` ให้ครบถ้วนเสมอ** ก่อนนำไป Destructure ใน Custom Hook (เช่น `import { newApi } from './api'`) เพราะถ้าลืม React จะแคร์ชทั้งแอปพลิเคชันทันที
